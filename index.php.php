...
function obter_conteudo()
{
    ...
    foreach($links as $link){ // Passar por todas as páginas e coletar informações
        $file = str_get_html(file_get_contents($link)); // Coletando página e transformando em String.
        $resultado = filtro("div","class=\"\" id=\"parent-fieldname-(.*?)\"",$file); // XML para coletar somente o conteúdo importante
        $licitacoes[] = "<hr><br/><a id='paglink' href='$link'>Página: $link</a><br/>".$resultado; // Colando Link, para o destino da página ao qual o conteúdo pertence.
    }
